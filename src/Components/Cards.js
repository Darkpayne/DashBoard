const Cards = () => {
  const Data = [
    ...
    {
      image: 5,
      link: "worktime.abiliintegratedserv.org/admin/transactions/transfer",
      svg: "cash-outline",
      title: "Accounting",
    ...
    <div className="grid mx-2 md:mx-0 xl:grid-cols-3 lg:grid-cols-3 grid-cols-1 md:grid-cols-2 gap-y-10 ">
      {Data.map((card, index) => {
        return (
          <div
            key={index}
            class="col-span-1 mx-auto justify-center rounded-2xl shadow-xl shadow-slate-300/60 hover:scale-105 duration-500 translate-y-1 cursor-pointer active:scale-90 w-[300px] text-center bg-white"
          >
            <a href={`https://${card.link}`}>
              ...
              <div class="p-4 bg-slate-50">
                <h1 class="text-xl font-medium text-slate-800 pb-2 capitalize">
                  {card.title}
                </h1>
                <p class="text-sm font-semibold text-slate-500 capitalize">
                  {card.desc}
                </p>
              </div>
            </a>
